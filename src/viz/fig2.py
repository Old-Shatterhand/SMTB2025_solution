...
from src.viz.constants import DATASET_COLORS, DATATASK_NAMES
...
def plot_fig2(models):
    ...
    for m, metric in enumerate(["Neighborhood Overlap", "2NN ID", "Variance @ 10"]):
        for d in range(4):
...
def plot_full_fig2_top(models):
    ...
    METRIC_NAMES = [r"Neighborhood Overlap $\chi_{10}^{l,l+1}$", "2NN Intrindic Dimension", "Variance@10"]
    DS_NAME_MAP = {
        "fluorescence": "Fluorescence",
    ...
    axs[0][1].set_ylim(-0.05, 30.05)
    axs[1][1].set_ylim(-0.05, 25.05)
    axs[2][1].set_ylim(-0.05, 20.05)
    axs[3][1].set_ylim(-0.05, 15.05)
    for ax in axs:
...
def plot_full_fig2_bot(models):
    ...
    for m, metric in enumerate(["noverlap", "ids", "var@10"]):
        ...
        if m == 0:
            axs[-3][m].set_ylabel("SCOPe40 protein level")
            axs[-2][m].set_ylabel("SCOPe40 residue level")
