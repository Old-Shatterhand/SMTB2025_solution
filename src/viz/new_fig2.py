...
def correlate(ds1, ds2):
    ...
    for i in range(len(ds1)):
        coeffs.append(np.corrcoef(np.array(ds1[i]), np.array(ds2[i]))[0, 1])
...
def plot_new_fig2(models, algo: str = "lr", class_metric: str = "mcc", reg_metric: str = "pearson"):
    ...
    datasets = ["fluorescence_classification", "fluorescence", "deeploc2_bin", "deeploc2", "meltome_atlas_species", "meltome_atlas", "scope_40_208_3ssp", "scope_40_208_8ssp"]
    data = [
        [metrics[model][dataset] for model in models] for dataset in datasets
    ]
    ...
    for i in range(4):
        ...
        if i in {1, 3}:
            ...
            scnd_handles, scnd_labels = scnd.get_legend_handles_labels()
            handles.extend(scnd_handles)
    ...
    # axs[0].set_title(DATATASk_NAMES["fluorescence_classification"])
    ...
    # axs[1].set_title(DATATASk_NAMES["fluorescence"])
    ...
    # axs[2].set_title(DATATASk_NAMES["scope_40_208_3ssp"])
    ...
    # axs[3].set_title(DATATASk_NAMES["scope_40_208_8ssp"])
...
def plot_full_fig3_left(models: list[str] = MODELS, algo: str = "knn", class_metric: str = CLASS_METRIC, reg_metric: str = REG_METRIC):
    ...
    ip_data = interpolate_data(data)

    for i in range(4):
        axs[i].plot(XP, np.nanmean(ip_data[2 * i], axis=0), label=DATATASK_NAMES[datasets[2 * i]].split(" ")[-1 if i != 3 else -2], linewidth=5)
        axs[i].fill_between(XP, np.nanmin(ip_data[2 * i], axis=0), np.nanmax(ip_data[2 * i], axis=0), alpha=0.2)
        axs[i].grid()
        if i != 3:
            axs[i].set_title(" ".join(DATATASK_NAMES[datasets[2 * i]].split(" ")[:-1]))
        else:
        ...
        if i in {1, 3}:
            axs[i].plot(XP, np.nanmean(ip_data[2 * i + 1], axis=0), label=DATATASK_NAMES[datasets[2 * i + 1]].split(" ")[1], linewidth=5)
            axs[i].fill_between(XP, np.nanmin(ip_data[2 * i + 1], axis=0), np.nanmax(ip_data[2 * i + 1], axis=0), alpha=0.2)
            axs[i].legend(loc="lower center")
        else:
            scnd = axs[i].twinx()
            scnd.plot(XP, np.nanmean(ip_data[2 * i + 1], axis=0), label=DATATASK_NAMES[datasets[2 * i + 1]].split(" ")[-1], color="tab:orange", linewidth=5)
            scnd.fill_between(XP, np.nanmin(ip_data[2 * i + 1], axis=0), np.nanmax(ip_data[2 * i + 1], axis=0), alpha=0.2, color="tab:orange")
            scnd.set_ylabel(METRIC_TITLES[TASK_METRICS[DATASET2TASK[datasets[2 * i + 1]]]])
    ...
    plt.tight_layout()
    plt.savefig(f"paper_figures/full_3_{algo}_{class_metric}_{reg_metric}.pdf", dpi=300, bbox_inches="tight")
...
def plot_full_fig3_right():
    ...
    for d, dataset in enumerate(["fluorescence_classification", "fluorescence", "meltome_atlas", "deeploc2_bin", "deeploc2", "stability", "scope_40_208_fold", "scope_40_208_superfamily", "solubility"]):
        ...
        axs[d].set_xticks(RATIOS)
        axs[d].set_title(DATATASK_NAMES[dataset])
        axs[d].grid()
